...
class ChartOfAccountsController extends Controller
{
    ...
    public function newAccount(Request $request, Account $account)
    {
        ...
        if( $request->isMethod('post') )
        {
            ...
            $account->unit_cost = $request->input('unit_cost');
            $account->unit_price = $request->input('unit_price');
            $account->min_qty = $request->input('min_qty');
    ...
    public function show($account_id)
    {
        ...
        $data['min_qty'] = $account_data->min_qty;
        $data['unit_price'] = $account_data->unit_price;
        $data['unit_cost'] = $account_data->unit_cost;
    ...
    public function modify( Request $request, $account_id, Account $account )
    {
        ...
        if( $request->isMethod('post') )
        {
            ...
            $account->unit_price = $request->input('unit_price');
            $account->min_qty = $request->input('min_qty');
            $account->account_code = $request->input('account_name');
            $account->save();
